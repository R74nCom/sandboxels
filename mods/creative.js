//Credit to R74n for the mod's functionality! I just made a joke about it
...
elements.cloner.tick = function(pixel) {
    if (settings.survivalClone) {
        if (Math.random() < 0.9999999999999999999999999999999999999999999999999999) {
            // 1 or -1
...
elements.greedy_sell = {
    color: elements.diamond.color
    tool: function(pixel) {
        if (elementWorth[pixel.element] === 0) { return; }
        deletePixel(pixel.x,pixel.y);
        if (elementWorth[pixel.element] === -1) { return; }
        survivalAdd("diamond",elementWorth[pixel.element]||1);
    },
    toolHoverStat: function(pixel) {
        return "$"+(elementWorth[pixel.element]||1);
    },
...
elements.illegal_sell = {
    color: elements.gravitite.color
    tool: function(pixel) {
        if (elementWorth[pixel.element] === 0) { return; }
        deletePixel(pixel.x,pixel.y);
        if (elementWorth[pixel.element] === -1) { return; }
        survivalAdd("gravitite",elementWorth[pixel.element]||1);
    },
    toolHoverStat: function(pixel) {
        return "$"+(elementWorth[pixel.element]||1);
    },
    category: "tools",
    desc: "Exchanges pixels for their market value in Gravitite"
}
